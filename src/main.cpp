...
GLuint BuildTriangles2(); // Constrói triângulos para renderização
...
glm::vec4 cam_position = glm::vec4(15.0f, 1.0f, 10.0f, 1.0f);
...
bool jumping_dir = 1.0f;
bool jumping_speed = 0.05f;

float world[2000][2000][4];
bool block_exists[2000][2000];
...
void generate_world() {
    for (int i = 0; i < 2000; i++) {
        for (int j = 0; j < 2000; j++) {
            world[i][j][0] = i;
            world[i][j][1] = j;
            world[i][j][2] = round(85 * float(ValueNoise_2D(i, j)));

            world[i][j][3] = float(ValueNoise_2D(i, j));
            // printf("%f\n", world[i][j][3]);
            block_exists[i][j] = true;
        }
    }
}
...
Player player = Player(5.0f, 0.0f, 0.0f);

int main()
{
    //generate_world();
    GameWindow gameWindow(800, 800, "fcg-glcraft");
    gameWindow.init();
    ...
    player.gameObject->SetKeyManager(&keyManager);
    ...
    GLuint vertex_array_object_id2 = BuildTriangles2();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        glm::vec4 camera_position_c = cam_position; // Ponto "c", centro da câmera
        ...
        int player_x = round(cam_position.x);
        int player_y = cam_position.y;
        int player_z = round(cam_position.z);
        bool hasBlockUnderPlayer = block_exists[player_x][player_z];


        is_grounded = true;
        /* if (hasBlockUnderPlayer && player_y > float(world[player_x][player_y][2]) + playerHeight) {
             is_grounded = false;

                 cam_position.y -= gravity;

         } else if (!hasBlockUnderPlayer) {
             cam_position.y -= gravity;
         }

         if (cam_position.y < float(world[player_x][player_y][2]) + playerHeight) {
             cam_position.y = float(world[player_x][player_y][2]) + playerHeight;
         }*/
        ...
        // aqui dps
        if (keyManager.W_DOWN) {

            cam_position = cam_position + -w * SPEED * speed_multiplier;
        }
        if (keyManager.S_DOWN) {

            cam_position = cam_position + w * SPEED * speed_multiplier;
        }
        if (keyManager.D_DOWN) {

            cam_position = cam_position + u * SPEED * speed_multiplier;
        }
        if (keyManager.A_DOWN) {

            cam_position = cam_position + -u * SPEED * speed_multiplier;
        }


        if (is_jumping && jumping_dir == 1.0f) {
            cam_position.y += jumping_speed;
            if (cam_position.y >= groundHeight + playerHeight + 5.0f) {
                jumping_dir = -1.0f;
                is_jumping = false;
            }
        }
        ...
        float x_minus = -100.0f;
        float x_plus = 100.0f;
        float z_minus = -100.0f;
        float z_plus = 100.0f;

        if (player_x - x_minus < 0) {
            x_plus = x_plus - (player_x - x_minus);
            x_minus = 0;
        }
        if (player_x + x_plus >= 2000) {
            x_plus = 0;
        }

        if (player_z - z_minus < 0) {
            z_plus = z_plus - (player_z - z_minus);
            z_minus = 0;
        }
        if (player_z + z_plus >= 2000) {
            z_plus = 0;
        }


        // Vamos desenhar 3 instâncias (cópias) do cubo
        for (int i = 0; i < player_x + x_plus; ++i)
        {


            for (int j = 0; j < player_z + z_plus; j++) {
                if (world[i][j][3] > 0.0f) {
            ...
            // slides 2-14 e 184-190 do documento Aula_08_Sistemas_de_Coordenadas.pdf.
                ...
                model = Matrix_Translate(world[i][j][0], groundHeight + world[i][j][2], world[i][j][1]);
...
GLuint BuildTriangles()
{
    ...
    GLfloat color_coefficients[] = {
        ...
        //  R     G     B     A
            0.0f, 0.1f, 0.0f, 1.0f, // cor do vértice 0
            0.0f, 0.2f, 0.0f, 1.0f, // cor do vértice 1
            0.0f, 0.3f, 0.0f, 1.0f, // cor do vértice 2
            0.0f, 0.4f, 0.0f, 1.0f, // cor do vértice 3
            0.0f, 0.5f, 0.0f, 1.0f, // cor do vértice 4
            0.0f, 0.6f, 0.0f, 1.0f, // cor do vértice 5
            0.0f, 0.7f, 0.0f, 1.0f, // cor do vértice 6
...
GLuint BuildTriangles2()
{
    ...
    GLfloat color_coefficients[] = {
        // Cores dos vértices do cubo
        //  R     G     B     A
            0.51f, 0.28f, 0.0f, 1.0f, // cor do vértice 0
            0.52f, 0.29f, 0.0f, 1.0f, // cor do vértice 1
            0.53f, 0.30f, 0.0f, 1.0f, // cor do vértice 2
            0.54f, 0.31f, 0.0f, 1.0f, // cor do vértice 3
            0.55f, 0.32f, 0.0f, 1.0f, // cor do vértice 4
            0.56f, 0.33f, 0.0f, 1.0f, // cor do vértice 5
            0.57f, 0.34f, 0.0f, 1.0f, // cor do vértice 6
            0.58f, 0.35f, 0.0f, 1.0f, // cor do vértice 7
        // Cores para desenhar o eixo X
            1.0f, 0.0f, 0.0f, 1.0f, // cor do vértice 8
            1.0f, 0.0f, 0.0f, 1.0f, // cor do vértice 9
        // Cores para desenhar o eixo Y
...
void KeyCallback(GLFWwindow* window, int key, int scancode, int action, int mod)
{
    ...
    if (key == GLFW_KEY_W) {
        keyManager.W_DOWN = action;
    }

    if (key == GLFW_KEY_S) {
        keyManager.S_DOWN = action;
    }

    if (key == GLFW_KEY_A) {
        keyManager.A_DOWN = action;
    }

    if (key == GLFW_KEY_D) {
        keyManager.D_DOWN = action;
    }
    ...
    if (key == GLFW_KEY_SPACE && action == GLFW_PRESS && is_grounded) {
        is_jumping = true;
        jumping_dir = 1.0f;
    }
